...
class WorkcrawlerSpider(scrapy.Spider):
    ...
    def historyRecord(self, log_record):
        ...
        if("price" in log_record and "time_ymd" in log_record and "amount" in log_record):
            single_time_stamp["price"] = log_record["price"]
            single_time_stamp["amount"] = log_record["amount"]
    ...
    def parse(self, response, workID, extra_info):
        ...
        for option in options:
            ...
            link_urls = [(option_str, idx, "https://dojindb.net/w/{0}?mode=getgraph&g={1}&site={2}".format(workID, option_str,idx)) for idx in range(1,len(item["release_dtl"])+1)]
    ...
    def parse_history(self, response):
        ...
        if(link_data["log"]):
            ...
            single_history_data["price_sum"] = link_data["price_sum"]
            single_history_data["amount_sum"] = link_data["amount_sum"]
            # yielding data into item
            site_idx = response.meta["site_idx"]
            period = response.meta["period"]
            if period not in item["historyData"]:
                item["historyData"].update({period:{}})
            item["historyData"][period][site_idx] = single_history_data
