...
class WorkcrawlerSpider(scrapy.Spider):
    ...
    def has_duplicate_start(self, lst):
        seen = set()
        for item in lst:
            if item[0] in seen:
                return True
    ...
    def parse(self, response, workID, extra_info):
        ...
        for sitediff in release_info:
            ...
            if 'www.dmm.co.jp' == parts[2]:
                ID_seiral = parts[-2].split("=")[1]
            elif 'www.dlsite.com' == parts[2]:
                ID_seiral = parts[-1].split(".")[0]
            item["release_dtl"][ID_seiral] = {'work price':work_price,'date':released_date,'sales':sales}
    ...
    def parse_links(self, response, item, historyData):
        ...
        if historyData:
            ...
            id_in_data = item["controversial"]
            item["verified"] = list(id_in_data.intersection(id_in_page))
            if(self.has_duplicate_start(item['verified'])):
                item["controversial"] = list(id_in_data.union(id_in_page))
                item['verified'] = []
            else:
                item["controversial"] = list(id_in_data.symmetric_difference(id_in_page))
            return item
    ...
    def historyRecord(self, log_record):
        ...
        return code, [str(price), str(amount), time_ymd, str(int(get_julian_day(time_ymd)))]
    ...
    def parse_history(self, response):
        ...
        if link_data.get("log"):
            ...
            for log_record in link_data["log"]:
                code, single_record = self.historyRecord(log_record)
                if single_record:
                    single_history_data.append((code, single_record))
            single_history_data_dict = {}
            for code, record in single_history_data:
                if code:
                    code_set.add(code)
                single_history_data_dict.setdefault(code, []).append(record)
            if "" in single_history_data_dict:
                if len(code_set) == 1:
                    single_history_data_dict[code_set.pop()].extend(single_history_data_dict[""])
                    del single_history_data_dict[""]
                elif not code_set:
                    code = next((key for key in item['release_dtl'] if (site_idx == 1 and key[0] == 'd') or (site_idx == 2 and key[0] == 'R')), 'Missing')
                    single_history_data_dict[code] = single_history_data_dict.pop("")
                else:
                    print("Multiple Codes encounter at {0}".format(item["ID"]))
                    raise ValueError
            period = response.meta["period"]
            item["controversial"] = item["controversial"].union(single_history_data_dict)
            item["historyData"].setdefault(period, {})[site_idx] = single_history_data_dict
        # process items left
