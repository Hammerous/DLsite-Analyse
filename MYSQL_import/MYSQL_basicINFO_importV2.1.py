...
def process_array(data):
    ...
    if duplicate_indices.size:
        for group in np.split(duplicate_indices, np.where(np.diff(duplicate_indices) != 1)[0]+1):
            amount_arr = data[np.insert(group,0,group[0]-1),2]
            if not np.all(amount_arr == amount_arr[0]):
                raise ValueError("Not all amount equals!!!")
        data = np.delete(data, duplicate_indices, axis = 0)
        data_idx = np.delete(data_idx, duplicate_indices)
...
def record_extract(doujin_id, info, is_daily):
    ...
    return {
        code_id: [
            (is_daily, each[2], doujin_id, code_id, int(each[3]), int(float(each[0])), int(float(each[1])))
            for each in detail
        ]
...
def insert_sales_history(ref_code_dict, historyData, doujin_id, time_lst):
    ...
    result_lst = []
    for values in valid_time_dict.values():
        this_records = np.array(values)
...
if __name__=='__main__':
                ...
                for valid_id in v_lst:
                    ...
                    if(valid_id[0] == 'd'):
                        ...
                        if(valid_id in insert_RJs):
                            ...
                            if(each["Reviews_info"]):
                                Tags_dict.update(each["Reviews_info"])
                            if(each["Series"]):
    ...
    for key, value in list_dict.items():
        if(value):
            ...
            list_dict[key] = []
    '''
    for key, value in deposited_ids_dict:
        if key[0] == 'd' and key not in DM2doujin_dict:
            title = value['Title']
            if title in reverse_main_dict:
                main_id = reverse_main_dict[title]
    '''
    if False_RJ_dict:
