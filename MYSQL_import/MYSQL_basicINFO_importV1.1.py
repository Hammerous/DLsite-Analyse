import pymysql,ijson,datetime,hashlib,binascii
...
def create_tables():
    ...
    CREATE TABLE doujinPrice (
                ...
                time_str VARCHAR(12) NOT NULL,
                jd_time INT UNSIGNED NOT NULL,
                price_DLsite INT UNSIGNED NULL,
                price_FANZA INT UNSIGNED NULL
    )
    """)
    db.commit()
...
def record_extract(info, dict, doujin_id):
    for date, info in info.items():
        date = int(date)
        if(info['code'] and info['amount'] != '0'):
            if date not in dict:
                dict[date] = (info['time_ymd'],doujin_id,info['code'],int(info['price']),float(info['amount']))
            elif if_integer(info['amount']):
                dict[date] = (info['time_ymd'],doujin_id,info['code'],int(info['price']),int(info['amount']))
...
def insert_sales_history(historyData, doujin_id, daily_lst, monthly_lst):
    ...
    for type, data in historyData.items():
        if(data):
            ...
            record_extract(data['log'], target_dict, doujin_id)
...
def insert_price_history(priceData, doujin_id, price_lst):
    priceinfo_lst = [
        (date2jd(time), time, doujin_id,
         None if dlsite == 'None' else int(dlsite),
         None if fanza == 'None' else int(fanza))
...
if __name__=='__main__':
    ...
    create_tables()
    pbar_record = tqdm(total=items_numbers, desc="Writing into DATABASE", bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}]',\
    ...
    list_command = {'doujinWork':"INSERT DELAYED INTO doujinWork (id, main_id ,`Rank`) VALUES (%s, %s, %s);", 
                    'daily_record':"""
                    ...
                    'monthly_record':"""
                       ...
                       VALUES (%s, %s, %s, %s, %s, %s);
                       """, 
                    'price':"INSERT DELAYED INTO doujinPrice (jd_time, time_str, doujin_id, price_DLsite, price_FANZA) VALUES (%s, %s, %s, %s, %s);"
                    }

    with open(file_path, 'r', encoding='UTF-8') as f:
        for record in ijson.items(f, 'item'):
            singular_record = True
            if('historyData' not in record):
                singular_record = False
            if(singular_record):
                this_circle = [record['circle'].strip() if record['circle'] else '_missing_']
                this_title = record['title'].strip()
                #title_md5Byte = md5(this_title)
            ...
            if(singular_record):
                if(Work_main_id):
                    if(is_JPver):
                        ...
                        #reverse_main_dict[title_md5Byte] = reverse_main_dict.pop(md5(previous_title))
                        Work_main_dict[Work_main_id] = this_title
                #elif(title_md5Byte in reverse_main_dict):                                                         # [Performance Waring: this step takes a lot of computation]
                elif(this_title in reverse_main_dict):
                    ...
                    max_workID += 1
                    #reverse_main_dict[title_md5Byte] = max_workID
                    reverse_main_dict[this_title] = max_workID
                    Work_main_dict[max_workID] = this_title
                    Work_main_id = max_workID
                #cursor.execute("INSERT INTO doujinWork (id, main_id ,`Rank`) VALUES (%s, %s, %s);", \
                #                (record['ID'], Work_main_id, record['extra_info']['Rank'] if len(record['extra_info']) else None))
                list_dict['doujinWork'].append((this_doujinID, Work_main_id, record['extra_info']['Rank'] if len(record['extra_info']) else None))
                ...
                insert_price_history(record['price_data'], this_doujinID, list_dict['price'])
                insert_sales_history(record['historyData'], this_doujinID, list_dict["daily_record"], list_dict["monthly_record"])
                if(Child_RJexist):
                    cursor.execute("UPDATE RJWork SET doujin_id = %s, ref_price = %s WHERE id = %s",(this_doujinID, Child_RJ_price, each['Site']))
                    db.commit()
            else:
