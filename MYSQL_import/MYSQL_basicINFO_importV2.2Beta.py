import pymysql,ijson,datetime,re,json,codecs,os
...
def json_writein(filepath, json_data):
    ...
    item_json = json.dumps(json_data, ensure_ascii=False)
    ...
    for code_id, detail in info.items():
        if detail:
            ...
            if not_integer_idx.size:
                sub_arrays = np.split(arr, np.where(~not_integer_bool)[0])
                if not_integer_idx[0] == 0:
                    if sub_arrays[0].size == 1:
                        sub_arrays[0] = arr[:2]
                    slope = (sub_arrays[0][0]-sub_arrays[0][1])/(detail[0][5])
                    fraction = fractions.Fraction.from_float(slope).limit_denominator()
                    possible_range = np.arange(fraction.denominator)*slope + sub_arrays[0][0]
                    delta_arr = np.abs(possible_range - possible_range.astype(int))
                    min_idx = np.argmin(delta_arr)
                    if(np.max(delta_arr[min_idx]) > 1e-6):
                        raise ValueError("Record at {0} incorrect: {1}".format(code_id, detail))
                    date = int(detail[0][4]) + min_idx
                    detail = np.vstack((detail,np.array((is_daily,jd2date(date),doujin_id, code_id,\
                                                date, detail[0][5], possible_range[min_idx]))))
                                                # time_resolution, time_str, doujin_id, code_id, jd_time, price, amount
                if not_integer_idx[-1] == arr.size - 1:
                    slope = (sub_arrays[-1][-1]-sub_arrays[-1][-2])/2
                    fraction = fractions.Fraction.from_float(slope).limit_denominator()
                    possible_range = np.arange(fraction.denominator)*slope + sub_arrays[-1][-1]
                    delta_arr = np.abs(possible_range - possible_range.astype(int))
                    min_idx = np.argmin(delta_arr)
                    if(np.max(delta_arr[min_idx]) > 1e-6):
                        raise ValueError("Record at {0} incorrect: {1}".format(code_id, detail))
                    date = int(detail[-1][4]) - min_idx
                    detail = np.vstack((detail,np.array((is_daily,jd2date(date),doujin_id, code_id,\
...
def insert_sales_history(ref_code_dict, historyData, doujin_id, time_lst):
    ...
    for records in valid_time_dict.values():
        if records:
            ...
            arr = np.array(this_records[:,4:], dtype=float)
...
if __name__=='__main__':
    ...
    '''
    print("Altering DATABASE Engine...")
    cursor.execute('SHOW TABLES;')
    tables = cursor.fetchall()
    for table_name in tables:
        cursor.execute(f'ALTER TABLE {table_name[0]} ENGINE=InnoDB;')
    '''
