...
class DualsitesSpiderSpider(scrapy.Spider):
    ...
    terms_filter_sets = {'Age_Restrict','Genre','Other_info','language'}
    ...
    def parse_DLsite_workpage(self, response):
        language_info = response.xpath("//div[@class='work_edition_linklist type_trans']")
        if(len(language_info)):
            for versions_url in language_info.xpath(".//a/@href"):
                RJ_serial = versions_url.get().split("/")[-1].split(".")[0]
