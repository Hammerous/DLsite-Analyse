    ...
    def close_spider(self, spider):
        ...
        if not(os.path.exists(workdict_path)):
            ...
            for key, value in work_dict.items():
                all_ids.append({"ID" : key, "verified": value['verified'], "controversial": value['controversial']})
        ...
        missing_ids = []
        for ID, status_dict in work_dict.items():
            # if JSON file exists
            json_file_path = os.path.join(spider.crawl_folder_path, f"{ID}.json")
            if not os.path.exists(json_file_path):
                missing_ids.append({"ID" : ID, "verified": status_dict['verified'], "controversial": status_dict['controversial']})
                continue
            # reading JSON
            try:
            ...
            copy_dict = copy.deepcopy(status_dict)
            for each in json_data:
                copy_dict[each['Status']].remove(each['Site'])
            if(copy_dict['verified'] or copy_dict['controversial']):
                missing_ids.append({"ID" : ID, "verified": copy_dict['verified'], "controversial": copy_dict['controversial']})
