...
class DualsitesSpiderSpider(scrapy.Spider):
    ...
    def file_extract(self, json_path):
        ...
        with open(json_path, "r", encoding='utf-8') as f:
            for record in ijson.items(f, 'item'):
                ...
                self.url_sum += (len(v_lst)+len(c_lst))
                id_dict = {'verified': v_lst, 'controversial': c_lst} 
                self.work_dict[ID] = id_dict
    ...
    def start_requests(self):
        ...
        for ID, data in self.work_dict.items():
            for status, id_lst in data.items():
                for site_id in id_lst:
                    if site_id[0] == 'd':       #DMM serial
    ...
    def FANZA_Extract(self, term, info_item):
        if(term in self.FANZA_terms):
            ...
            if(item_type == 'Sub_Genre' or item_type == 'Released_date'):
    ...
    def parse_FANZA(self, response):
        ...
        if(len(basic_info)):
            ...
            Sales = level0_info.xpath(".//span[@class='numberOfSales__txt']/text()").get()
            if(Sales):
                item['Sales'] = Sales.replace(',','')
